...
public:
	...
	template <typename T>
	bool GetValue(size_t i, const std::string & c_rstrColKey, T& tValue) const;	// n th (map It's just the order in , txt It has nothing to do with the order of ) row A function that returns the value of a specific column of . 
																				// Although it is heterogeneous , Function for convenience .
	template <typename T>
