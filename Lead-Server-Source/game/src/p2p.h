...
class P2P_MANAGER : public singleton<P2P_MANAGER>
{
	public:
		...
		// under Register* Unregister* pair internally they actually use the same routines: .
		...
		void			EraseUserByDesc(LPDESC d);	// corresponding desc Delete users in .

		void			FlushOutput();

		void			Boot(LPDESC d);	// p2p Sends information necessary for processing . ( Login information for all characters, etc. )
