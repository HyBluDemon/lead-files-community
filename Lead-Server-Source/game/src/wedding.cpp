...
namespace marriage
{
	...
	void WeddingMap::SetEnded()
	{
		...
		for (itertype(m_set_pkChr) it = m_set_pkChr.begin(); it != m_set_pkChr.end(); ++it)
		{
			...
			if (ch->GetLevel() < 10) // 10 It will not be given if you are below level. .
				continue;
	...
	struct FWarpEveryone
	{
		void operator() (LPCHARACTER ch)
		{
			if (ch->IsPC())
			{
				// ExitToSavedLocation silver WarpSet In this function, we call
				// Sectree go NULL It becomes . later SectreeManager From
				// This character cannot be found, so DestroyAll Separately processed in
